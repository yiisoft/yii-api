...
次のような理由によって、リソースを [[yii\base\Model]] またはその子クラス (例えば [[Yiisoft\Db\ActiveRecord]])
...
最初に、リソースは [[Yiisoft\Yii\Rest\Serializer]] によって配列に変換されます。
...
[[Yiisoft\Db\ActiveRecord::extraFields()]] は DB から取得されたリレーションの名前を返します。
...
データプロバイダが RESTful API のレスポンスで送信される場合は、[[Yiisoft\Yii\Rest\Serializer]] が現在のページのリソースを取り出して、
リソースオブジェクトの配列としてシリアライズします。
それだけでなく、[[Yiisoft\Yii\Rest\Serializer]] は次の HTTP ヘッダを使ってページネーション情報もレスポンスに含めます。
