...
Не существует никаких ограничений на то, как представить ресурс, но в Yii ресурсы обычно представляются
как объекты [[yii\base\Model]] или дочерние классы (например [[Yiisoft\Db\ActiveRecord]]), потому как:
...
* [[Yiisoft\Db\ActiveRecord]] даёт мощную поддержку работы с БД, что актуально, если данные ресурса хранятся в ней.
...
Когда ресурс включается в ответ RESTful API, необходимо сериализовать его в строку. В Yii этот процесс разбит на два этапа.
Сначала ресурс конвертируется в массив при помощи [[Yiisoft\Yii\Rest\Serializer]]. На втором этапе массив сериализуется в строку
...
пользователь может запросить опционально через параметр `expand`:
...
[[Yiisoft\Db\ActiveRecord::fields()]] возвращает только те атрибуты, которые были объявлены в схеме БД.
...
По умолчанию, [[yii\base\Model::extraFields()]] ничего не возвращает, а [[Yiisoft\Db\ActiveRecord::extraFields()]]
...
Ключевой момент HATEOAS заключается в том, чтобы возвращать вместе с данными набор гиперссылок, указывающих на связанную
с ресурсом информацию.

Поддержку HATEOAS в классы ресурсов можно добавить реализовав интерфейс [[yii\web\Linkable]]. Этот интерфейс
содержит единственный метод [[yii\web\Linkable::getLinks()|getLinks()]], который возвращает список [[yii\web\Link|ссылок]].
Обычно необходимо вернуть как минимум ссылку `self` с URL самого ресурса:
...
Для RESTful APIs, которые работают с коллекциями, эти возможности используются довольно часто. Например, следующее
...
При отправке ответа RESTful API, [[Yiisoft\Yii\Rest\Serializer]] сериализует массив объектов ресурсов для текущей страницы.
...
Примеры вы можете найти в разделе «[Быстрый старт](quick-start.md#trying-it-out)».
