...
Если ваши контроллеры унаследованы от [[Yiisoft\Yii\Rest\ActiveController]], вы можете переопределить
метод [[Yiisoft\Yii\Rest\Controller::checkAccess()|checkAccess()]] для выполнения авторизации. Этот метод будет вызываться
встроенными действиями, предоставляемыми контроллером [[Yiisoft\Yii\Rest\ActiveController]].
