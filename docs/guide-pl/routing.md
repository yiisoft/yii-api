...
Porównując to z menadżerem URLi dla aplikacji Web, główną nowością tutaj jest użycie [[Yiisoft\Rest\UrlRule|UrlRule]] do routingu RESTfulowych zasobów API. 
...
Dzieje się tak, ponieważ [[Yiisoft\Rest\UrlRule|UrlRule]] automatycznie przechodzi na formę mnogą dla ID kontrolerów podczas tworzenia potomnych zasad URL.
Zachowanie to można wyłączyć ustawiając [[Yiisoft\Rest\UrlRule::pluralize|pluralize]] na `false`. 
...
[[Yiisoft\Rest\UrlRule::controller|controller]], aby bezpośrednio określić w jaki sposób nazwa użyta w punkcie końcowym URLi ma być zmapowana na ID kontrolera. 
