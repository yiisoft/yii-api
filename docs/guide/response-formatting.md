...
2. Convert resource objects into arrays, as described in the [Resources](rest-resources.md) section.
   This is done by [[Yiisoft\Rest\Serializer]].
...
controller class [[Yiisoft\Rest\Controller]] is equipped with this filter under the name of `contentNegotiator`.
...
[[Yiisoft\Rest\Serializer]] will convert the result into an array. And finally, [[yii\web\JsonResponseFormatter]]
...
As we have described above, [[Yiisoft\Rest\Serializer]] is the central piece responsible for converting resource
...
You may configure the serializer by setting the [[Yiisoft\Rest\Controller::serializer]] property with a configuration array.
For example, sometimes you may want to help simplify the client development work by including pagination information
directly in the response body. To do so, configure the [[Yiisoft\Rest\Serializer::collectionEnvelope]] property
as follows:
